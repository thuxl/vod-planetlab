...
int main()
{
	...
	while (i < num && active_i < NUM_OF_PEERS) {
		...
		printf("%s\n",inet_ntoa(*((struct in_addr *) nlp_host->h_addr)));
		printf("#%s\n", hostname[i]);
		...
		if (i == start_i) {
			...
			printf("const char* 		   server_ip  = \"166.111.70.5\";\n");
			//printf("const char* 		   server_ip  = \"131.247.2.242\";\n"); //"planetlab2.csee.usf.edu",
			printf("const int 		   	   server_port = TRANS_CONTROL_PORT;\n");
			printf("const int 		   	   peers_port[NUM_OF_PEERS] = {\nTRANS_CONTROL_PORT");
			for (int j=1; j<NUM_OF_PEERS; j++){
				printf(", TRANS_CONTROL_PORT");
				if (j%6 == 0)  printf("\n");
			}
			printf(" };\n");
			...
			//printf("i=0;\n");
			//printf("echo \"[$i] %s\"; i=`echo \"$i+1\" |bc`;\n", hostname[i]);

			//printf("pid=`ssh -l tsinghua_leixu -i id_rsa %s \"netstat -tulpn|grep p_peer000|cut -d' ' -f57|cut -d'/' -f1\"`\n", hostname[i]);
			//printf("if [ -z \"$pid\" ]\nthen\n\techo \"no peer000 run.\"\nelse\n\techo \"kill pid=$pid\"\n");
			//printf("\tssh -l tsinghua_leixu -i id_rsa %s  \"kill $pid\"\nfi\n", hostname[i]);

			//printf("scp -i id_rsa p_peers/p_peer%03d tsinghua_leixu@%s:~/p_peer000\n\n", active_i, hostname[i]);
		}
		else{
			//================================  all_hostname.cc
			...
			if (active_i == NUM_OF_PEERS - 1) {
				printf(");\n\nfor((i=0;i<%d;i++));do\n", NUM_OF_PEERS);
				printf("{\n\tfilename=`printf \"p_peers/p_peer%%03d\" $i`\n");
				printf("\techo \"[$i] ${ips[$i]} $filename\"\n");
				printf("\tscp -i id_rsa $filename tsinghua_leixu@${ips[$i]}:~/p_peer000\n}&\ndone\nwait\n");
			}
			//scp -i id_rsa p_peer000 tsinghua_leixu@pl1.6test.edu.cn:~/p_peer000
			//printf("echo \"[$i] %s\"; i=`echo \"$i+1\" |bc`;\n", hostname[i]);
			//printf("pid=`ssh -l tsinghua_leixu -i id_rsa %s \"netstat -tulpn|grep p_peer000|cut -d' ' -f57|cut -d'/' -f1\"`\n", hostname[i]);
